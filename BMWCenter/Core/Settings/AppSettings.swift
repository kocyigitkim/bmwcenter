...
final class AppSettings {
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        vehicleName = defaults.string(forKey: Keys.vehicleName) ?? "BMW"
        fuelType = FuelType(rawValue: defaults.string(forKey: Keys.fuelType) ?? "") ?? .gasoline
        tankCapacityL = defaults.object(forKey: Keys.tankCapacityL) as? Double ?? 60.0
        displacementL = defaults.object(forKey: Keys.displacementL) as? Double ?? 2.0
        volumetricEfficiency = defaults.object(forKey: Keys.volumetricEfficiency) as? Double ?? 0.85
        isTurbo = defaults.object(forKey: Keys.isTurbo) as? Bool ?? true
        vehiclePlatform = VehiclePlatform(rawValue: defaults.string(forKey: Keys.vehiclePlatform) ?? "")
            ?? .bmwF30N13
