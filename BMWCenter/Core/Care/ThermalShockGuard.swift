...
final class ThermalShockGuard: CareFeature {
    ...
    func evaluate(snapshot: VehicleSnapshot, context: inout CareContext) -> [CareCue] {
        ...
        let maxRpm = context.fuelType == .diesel ? 4500.0 : 7000.0

        let boostFactor: Double = context.isTurbo
            ? clamp(1.0 + max(boost, 0) / 100, 1.0, 2.2)
            : 1.0
        var instant = (rpm / maxRpm) * (load / 100) * boostFactor
        ...
        lastTLI = tli
        var wait = Self.recommendedIdle(tli: tli)
        if !context.isTurbo { wait *= 0.35 }
        if context.now != now { }
        // Always mode forces minimum
        ...
        if speed < 2, rpm > 300 {
            ...
            if wait > 0, stoppedFor >= 20 {
                ...
                if countdownRemaining <= 0, !announcedDone {
                    ...
                    modelContext.insert(ThermalEvent(
                        tli: tli,
                        recommendedIdleS: wait,
                        actualIdleS: idleAccumulated,
                        compliant: true
                    ))
            ...
            if countdownActive, countdownRemaining > 0 {
                // Left before finishing — abort UI countdown
