...
final class OverheatWatchdogWarmupTests: XCTestCase {
    ...
    func testWarmedUpRapidRiseWhileIdleStillTriggersFanSuspect() {
        ...
        // Get the engine to just-warmed-up baseline first (86°C, stable).
        ...
        _ = watchdog.evaluate(snapshot: snapshot(coolantC: 86, speedKmh: 0), context: &context)

        // Now a real rise from an already-warm baseline while idling — this
        // is the scenario the fan-suspect check exists to catch.
        var allCues: [CareCue] = []
        for step in 1...9 {
            let t = start.addingTimeInterval(20 + Double(step) * 10)
            context.now = t
            let coolant = 86.0 + Double(step) * (7.0 / 9.0)
            allCues += watchdog.evaluate(snapshot: snapshot(coolantC: coolant, speedKmh: 0), context: &context)
        }
        XCTAssertTrue(
            allCues.contains { $0.id == "overheat.fan" },
