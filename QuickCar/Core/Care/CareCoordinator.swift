...
final class CareCoordinator: ObservableObject {
    ...
    private var cumulativeMinutes: Double = 0

    init(settings: AppSettings, modelContext: ModelContext, announcer: AudioAnnouncer, profileStore: VehicleProfileStore) {
    ...
    func evaluate(snapshot: VehicleSnapshot, supportedPIDs: Set<UInt8>, now: Date = .now) {
        ...
        context.profile = profileStore.profile
        let gate = signalGate.evaluate(snapshot: snapshot, profile: context.profile, now: now)
        context.canHealthy = gate.canHealthy
        context.normLoad = gate.normLoad
        context.effectiveAmbientC = gate.effectiveAmbientC
        context.regenActive = gate.regenActive
        context.confidenceTier = .forTrips(tripCount, minutes: cumulativeMinutes, hasMatureBaseline: tripCount >= 30)
    ...
    func onTripEnded(_ trip: Trip) {
        ...
        tripCount += 1
        cumulativeMinutes += trip.durationS / 60
        streak.recordDay(
