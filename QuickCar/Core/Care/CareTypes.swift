...
struct CareContext: Sendable {
    ...
    // Kural Seti v2 — araç profili duyarlılığı
    var profile: VehicleDiagnosticProfile = VehicleArchetypeDefaults.profile(for: .gasolineTurboDI, fuel: .gasoline, isTurbo: true)
    var confidenceTier: ProfileConfidenceTier = .t0
    var normLoad: Double?
...
/// §2 — profil güven seviyeleri: T0 sadece critical, T1 alarm açar, T2 watch+coach açar.
enum ProfileConfidenceTier: Int, Sendable, Comparable {
    case t0, t1, t2

    static func < (lhs: ProfileConfidenceTier, rhs: ProfileConfidenceTier) -> Bool { lhs.rawValue < rhs.rawValue }

    static func forTrips(_ tripCount: Int, minutes: Double, hasMatureBaseline: Bool) -> ProfileConfidenceTier {
        if hasMatureBaseline, tripCount >= 30 { return .t2 }
        if tripCount >= 3, minutes >= 40 { return .t1 }
        return .t0
    }
}
