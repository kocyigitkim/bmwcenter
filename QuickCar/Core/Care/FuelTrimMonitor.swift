...
final class FuelTrimMonitor: CareFeature {
    ...
    func evaluate(snapshot: VehicleSnapshot, context: inout CareContext) -> [CareCue] {
        ...
        guard profile.fuel != .diesel, profile.o2Type != .none else { return [] }
        guard !profile.flags.contains("fuelTrimDisabled"), !context.regenActive else { return [] }
        guard let ltft = snapshot.ltftBank1, context.canHealthy else { return [] }
        ...
        if let refuel = context.lastRefuelAt, context.now.timeIntervalSince(refuel) < 600 {
            return []
        }
        guard coolant > profile.thermostatOpenC.value - 5, fuel > 15 else { return [] }
        // DFCO proxy (decel fuel cutoff — closed throttle, engine braking): trim not representative.
        guard !(normLoad < 5 && rpm > 1200) else { return [] }

        let band: String
        switch (speed, normLoad) {
        case (...3, ...25): band = "idle"
        ...
        let altitudeBucket = profile.altitudeM > 1200 ? "alt:high" : "alt:low"
        baseline.observe(
            key: "ltft.\(band)", value: total, bucketKey: altitudeBucket,
            minSamples: 60, range: -40...40, now: context.now
        )
        var acc = bandAccum[band] ?? (0, 0)
        let ethanolAdj: Double = profile.ethanolBlend > 0.10 ? -min((profile.ethanolBlend - 0.10) * 40, 5) : 0
        acc.sum += total + ethanolAdj
        acc.n += 1
        bandAccum[band] = acc
    ...
    func onTripEnded(trip: Trip, context: CareContext) -> [CareCue] {
        ...
        var pattern: String?
        if let idle = idleAvg, idle > 20, (partAvg ?? 0) <= 10, (highAvg ?? 0) <= 10 {
            pattern = "lean_idle"
        } else if let idle = idleAvg, idle > 20, (partAvg ?? 0) > 20 {
            pattern = "lean_all"
        } else if let high = highAvg, high > 20, (idleAvg ?? 0) <= 10 {
            pattern = "lean_high"
        } else if let idle = idleAvg, idle < -20, (partAvg ?? 0) < -20 {
            pattern = "rich_all"
        }
        ...
        modelContext.insert(ProtectionEvent(
            typeRaw: "fuelTrim", severityRaw: "alarm",
            value: idleAvg ?? partAvg ?? highAvg ?? 0, thresholdUsed: 20
        ))
        try? modelContext.save()

        let drifting = LocalizedCare.string("trim.drifting")
        return [CareCue(
            id: "trim.drift", text: drifting, severity: .coach, localizationKey: "trim.drifting"
        )]
    }
