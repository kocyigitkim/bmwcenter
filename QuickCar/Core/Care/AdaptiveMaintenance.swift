...
final class AdaptiveMaintenance: CareFeature {
    ...
    let optionalPIDs: Set<UInt8> = [0x10, 0x0B, 0x0F]
    ...
    func evaluate(snapshot: VehicleSnapshot, context: inout CareContext) -> [CareCue] {
        ...
        if rpm > profile.redlineRpm * 0.6 { highRevS += dt }
        if speed < 2, rpm > 300 { idleS += dt }
        if let coolant = snapshot.coolantC, coolant > profile.coolantAlarmC - 3 {
            thermalStressMin += dt / 60
        }
        if let oil = context.oilTempC, oil >= profile.oilWarmC { sawHotOil = true }

        var cues: [CareCue] = []
        guard !context.regenActive else { return [] }

        if profile.airMeter == .speedDensity {
            if let map = snapshot.mapKpa, let rpm = snapshot.rpm, rpm > 300 {
                let bucket = CareBucket.composite(ambient: context.effectiveAmbientC ?? context.ambientC, load: nil, speed: speed)
                let ve = map * rpm
                baseline.observe(key: "ve.estimate", value: ve, bucketKey: bucket, minSamples: 300, range: 0...2_000_000, now: now)
                if let snap = baseline.snapshot(key: "ve.estimate", bucketKey: bucket), snap.isMature, ve < snap.p50 * 0.90 {
                    cues.append(airflowCue())
                }
            }
        } else if let maf = snapshot.mafGs, let load = snapshot.engineLoadPct {
            let baro = context.profile.altitudeM > 1200 ? "baro:high" : "baro:low"
            let iat = CareBucket.ambient(snapshot.intakeAirC)
            let bucket = [CareBucket.composite(ambient: context.effectiveAmbientC, load: load, speed: speed), baro, iat].joined(separator: "|")
            baseline.observe(key: "maf.flow", value: maf, bucketKey: bucket, minSamples: 300, range: 0...300, now: now)
            let threshold = profile.aspiration == .na ? 0.92 : 0.90
            if let snap = baseline.snapshot(key: "maf.flow", bucketKey: bucket), snap.isMature, maf < snap.p50 * threshold {
                cues.append(airflowCue())
            }
        }
    ...
    func onTripEnded(trip: Trip, context: CareContext) -> [CareCue] {
        ...
        let severity = Self.severityFactor(
            ...
            dieselDPFShortTrip: profile.hasDPF && trip.distanceKm < 8,
            directInjectionShortTrip: profile.injection == .direct && trip.distanceKm < 8
        )
        lastSeverity = severity
        applyLedger(distanceKm: trip.distanceKm, severity: severity, oilGrade: profile.oilGrade)
        resetTrip()
        return []
    }
    ...
    nonisolated static func severityFactor(
        ...
        avgSpeedKmh: Double,
        dieselDPFShortTrip: Bool = false,
        directInjectionShortTrip: Bool = false
    ) -> Double {
        var s = 1.0
        ...
        if dieselDPFShortTrip { s += 0.35 }
        if directInjectionShortTrip { s += 0.20 }
        return min(max(s, 1.0), 2.6)
    }
    ...
    private func applyLedger(distanceKm: Double, severity: Double, oilGrade: VehicleDiagnosticProfile.OilGrade) {
    ...
    private func airflowCue() -> CareCue {
        CareCue(id: "airflow.low", text: LocalizedCare.string("airflow.low"), severity: .coach, localizationKey: "airflow.low")
    }
