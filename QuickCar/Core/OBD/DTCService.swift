...
final class DTCService {
    ...
    private var allEntriesCache: (lang: String, rows: [(code: String, summary: String, severity: String, bmw: Bool)])?
    ...
    init(transportProvider: @escaping () -> OBDTransport?) {
        ...
        Task { [weak self] in
            let decoded = await Self.parseCatalogFile()
            self?.catalog = decoded
        }
    }

    /// Reads + decodes the ~2.2 MB / 9.9k-entry catalog off the main actor so the
    /// first access (typically the user opening the Stored DTC screen) doesn't hitch.
    private nonisolated static func parseCatalogFile() async -> [String: DTCCatalogEntry] {
    ...
    /// All catalog entries sorted — BMW-flagged first. Memoized per language since
    /// this scans/sorts ~9.9k entries and callers (search UI) may invoke it on every render.
    func allEntries(languageCode: String) -> [(code: String, summary: String, severity: String, bmw: Bool)] {
        if let cache = allEntriesCache, cache.lang == languageCode {
            return cache.rows
        }
        let rows = catalog
        ...
        allEntriesCache = (languageCode, rows)
        return rows
    }
