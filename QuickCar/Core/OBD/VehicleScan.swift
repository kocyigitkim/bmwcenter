...
enum VehicleScanOverallStatus: String, Sendable, Equatable {
    ...
    case attention
    case critical
    /// DTC yok ama readiness monitor'ları tamamlanmamış (batarya sökümü / son silme
    /// sonrası) — "temiz" demek yanlış olur, tarama tekrarlanmalı.
    case unknown
}
...
enum VehicleScanClassifier {
    static func classify(dtcs: [DTC], readiness: ReadinessStatus?) -> VehicleScanOverallStatus {
        ...
        if let readiness, !readiness.isFullyReady { return .unknown }
        return .good
    }
}
