...
final class CareFeatureTests: XCTestCase {
    ...
    func testColdCaps() {
        let c = ColdEngineShield.caps(oilTempC: 15, redlineRpm: 6500, diesel: false)
        XCTAssertNotNil(c.rpm)
        XCTAssertEqual(c.rpm!, 6500 * 0.35, accuracy: 0.01)
        XCTAssertEqual(c.load, 45)
        let hot = ColdEngineShield.caps(oilTempC: 90, redlineRpm: 6500, diesel: false)
    ...
    func testSeverityFactorBounds() {
        ...
        XCTAssertLessThanOrEqual(hard, 2.6)
    ...
    func testOverheatImmatureThresholds() {
        let profile = VehicleArchetypeDefaults.profile(for: .gasolineTurboDI, fuel: .gasoline, isTurbo: true)
        ...
        XCTAssertEqual(t.watch, profile.coolantWatchC, accuracy: 0.01)
        XCTAssertEqual(t.alarm, profile.coolantAlarmC, accuracy: 0.01)
        XCTAssertEqual(t.critical, profile.coolantCriticalC, accuracy: 0.01)
    }
